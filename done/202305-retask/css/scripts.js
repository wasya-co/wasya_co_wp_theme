(function($) {
  ...
  function sliderTestimonials() {
	...
	$(document).on('click', "input[name='status']", function(){
		...
		$("#form-fields-bottom").hide('slow');
		$('#response_message').html('');
		$("#email").val('');
	});
    
	$(document).on('click', '#sendy-status-check', function(){
		$('#response_message').html('');
		...
		var action = 'unubscribe_sendy';
		if ( status === 'Subscribe') {
			action = 'check_sendy_status';
		}
		...
		if( can_submit ){
			$.ajax({
				...
				success: function(response){
					...
					if ( response.update ) {
						$("#sendy-form-submit").val('UPDATE');
						$("#form-fields-bottom").show();
						$("#subscribe_action").val('update');
						
						$("#CompanyName").val(response.company);
						$("#FirstName").val(response.fname);
						$("#LastName").val(response.lname);
						$("#WorkPhone").val(response.wphone);
						$("#MobileWhatsApp").val(response.whphone);
						$("#Skills").val(response.skills);
					}
					if ( response.create ) {
						$("#sendy-form-submit").val('SUBMIT');
						$("#form-fields-bottom").show();
						$("#subscribe_action").val('create');
					}
					if (status == 'Subscribe') {
						$("#form-fields").show();
					}
    ...
    $(document).on('click', '#sendy-form-submit', function(){
		var $this = $(this);
		var $form = $this.parents('form');
		var can_submit = validate_form( $form );
		...
		if( can_submit ){
			$.ajax({
				...
				dataType: 'JSON',
				beforeSend:  function() {
					$(this).parent().find('.loader').css('display','inline-block');
				},
				success: function(response){
					$('#response_message').css('display','block');
					$('.loader').css('display','none');
					$('#response_message').html( response.message );//alert(response.success);
					if (response.success == '1') {
						$("#form-fields").hide('slow');
						$("#form-fields-bottom").hide('slow');
						$("#form-fields-code").hide();
						$("#email").val('');
						$("#CompanyName").val('');
						$("#FirstName").val('');
						$("#LastName").val('');
						...
						$("#subscribe_action").val('');
						$('#sent_code_message').html('');
					}
					if (response.create == '1') {
						$('#sent_code_message').html( response.sent_code );
						$("#form-fields-code").show();
						$("#sendy-form-submit").val('VERIFY');
						$("#subscribe_action").val('subscribe');
					}
				},
			});
		}
	...
	$(document).on('click', '#sendy-update-form-submit', function(){
		var $this = $(this);
		var $form = $this.parents('form');
		var can_submit = validate_form( $form );
	...
	$(document).on('click', '#sendy-unsubscribe-submit', function(){
		var $this = $(this);
		
		var data = {
			email : $("#email").val(),
			action: 'unubscribe_sendy_from_update',
		};
		...
		if( $this.val() !== ''){
			$.ajax({
				...
				success: function(response){
					...
					$("#myeditor").html(response.message);
					CKEDITOR.instances.myeditor.setData(response.message);
					$("#sendy-send-campaign-form-submit").val("SEND");
				},
			});
		} else {
			$("#subject").val('');
			CKEDITOR.instances.myeditor.setData('');
			$("#sendy-send-campaign-form-submit").val("SAVE & SEND");
		}
	});
	
	$(document).on('click', '#sendy-send-campaign-form-submit', function(){
		var $this = $(this);
		var $form = $this.parents('form');
		var can_submit = validate_form( $form );
	...
	$(document).on('click', '#pulse-login-form-submit', function(){
		var $this = $(this);
		var $form = $this.parents('form');
		var can_submit = validate_form( $form );
	...
	function validate_form( $container ){
		...
		$container.find('select, input, textarea').each(function(){
			...
			if( $$this.hasAttr('data-validation') && ( $$this.is(':visible') || ( $$this.attr('type') == 'hidden' ) ) ){
	...
	$(document).ready(function(){
        $("input[name='calendly']").click(function(){
            var radioId = $("input[name='calendly']:checked").attr('id');
            if(radioId){
				$(".calendly").hide();
				$("div#"+radioId).show();
            }
